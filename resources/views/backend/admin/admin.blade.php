            ...
            @can('system.admin.destroy')
				...
				if (ids.length > 0) {
                    layer.confirm('确认删除吗？', function(index) {
                        $.post("{{ route('admin.admin.destroy') }}", {_method: 'delete',_token:'{{csrf_token()}}',ids: ids}, function(res) {
                            if (res.code == 1) {
                                tableIns.reload();
                            }
                            layer.close(index);
        ...
        table.on('tool(currentTableId)', function (obj) {
			...
			if (obj.event === 'edit') {  // 监听添加操作
				...
				layer.confirm('真的删除行么', function (index) {
					$.post("{{ route('admin.admin.destroy') }}",{_method:'delete',_token:'{{csrf_token()}}',ids: [data.id]},function (res) {
                            if (res.code==1){
				...
				layer.confirm('真的恢复行么', function (index) {
					$.post("{{ route('admin.admin.restore') }}",{_method:'delete',_token:'{{csrf_token()}}',ids: data.id},function (res) {
                            if (res.code==1){
                                obj.del(); //删除对应行（tr）的DOM结构
								layer.close(index);
								layer.msg(res.msg,{icon:res.code});
