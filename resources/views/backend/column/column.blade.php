    ...
    <div class="layuimini-main">
        ...
        <script type="text/html" id="toolbarDemo">
            <div class="layui-btn-container">
				@can('site.column.destroy')
                <button class="layui-btn layui-btn-normal layui-btn-sm data-add-btn" lay-event="add"> 添加 </button>
                @endcan
                @can('site.column.create')
    ...
    layui.use(['form', 'table'], function () {
        ...
        var tableIns = table.render({
            ...
            cols: [[
                ...
                {field: 'title', title: '页面标题' ,width:250},
                {field: 'keywords', title: '页面关键词' ,width:400},
                {field: 'description', title: '页面描述' },
                {field: 'pic', title: '缩略图',width:150, templet:function(d){
        ...
        table.on('toolbar(currentTableId)', function (obj) {
            ...
            if (obj.event === 'add') {  // 监听添加操作
                var index = layer.open({
                    ...
                    shadeClose: false,
                    area: ['450px', '450px'],
                    content: '{{ route('site.column.create') }}',
					btn: ['确定', '取消'],
            ...
            @can('site.column.destroy')
				...
				if (ids.length > 0) {
                    layer.confirm('确认删除吗？', function(index) {
                        $.post("{{ route('site.column.destroy') }}", {_method: 'delete',_token:'{{csrf_token()}}',ids: ids}, function(res) {
                            if (res.code == 1) {
                                tableIns.reload();
                            }
                            layer.close(index);
        ...
        table.on('tool(currentTableId)', function (obj) {
			...
			if (obj.event === 'edit') {  // 监听添加操作
                var index = layer.open({
                    ...
                    shadeClose: false,
                    area: ['450px', '500px'],
                    content: '/admin/column/' + data.id + '/edit',
					btn: ['确定', '取消'],
					yes: function(index, layero){
					//点击确认触发 iframe 内容中的按钮提交
				...
				var index = layer.open({
                    ...
                    shadeClose: false,
                    area: ['450px', '600px'],
                    content: '/admin/column/' + data.id + '/role',
					btn: ['确定', '取消'],
					yes: function(index, layero){
					//点击确认触发 iframe 内容中的按钮提交
				...
				var index = layer.open({
                    ...
                    shadeClose: false,
                    area: ['800px', '650px'],
                    content: '/admin/column/' + data.id + '/permission',
					btn: ['确定', '取消'],
					yes: function(index, layero){
					//点击确认触发 iframe 内容中的按钮提交
				...
				layer.confirm('真的删除行么', function (index) {
					$.post("{{ route('site.column.destroy') }}",{_method:'delete',_token:'{{csrf_token()}}',ids: [data.id]},function (res) {
                            if (res.code==1){
				...
				layer.confirm('真的恢复行么', function (index) {
					$.post("{{ route('site.column.restore') }}",{_method:'delete',_token:'{{csrf_token()}}',ids: data.id},function (res) {
                            if (res.code==1){
                                obj.del(); //删除对应行（tr）的DOM结构
								layer.close(index);
								layer.msg(res.msg,{icon:res.code});
